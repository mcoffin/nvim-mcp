...
///
/// Uses the `dirs` crate to determine the appropriate runtime directory:
/// - On Unix-like systems: checks $XDG_RUNTIME_DIR, falls back to /tmp
/// - On Windows: uses appropriate temp directory
///
/// The runtime directory is used for storing socket files.
pub fn get_runtime_dir() -> String {
    // Try to get XDG runtime directory (Unix) or appropriate Windows temp dir
    if let Some(runtime_dir) = dirs::runtime_dir() {
        let runtime_path = runtime_dir.to_string_lossy().to_string();
        debug!("Using runtime directory: {}", runtime_path);
        runtime_path
    } else {
        // Fallback to /tmp on Unix or %TEMP% on Windows
        let fallback = if cfg!(target_os = "windows") {
            std::env::var("TEMP").unwrap_or_else(|_| "C:\\temp".to_string())
        } else {
            "/tmp".to_string()
        };
        debug!("Using fallback directory: {}", fallback);
        fallback
    }
}
...
pub fn find_get_all_targets() -> Vec<String> {
    ...
    let pattern = format!("{runtime_dir}/nvim-mcp.*.sock");
...
pub fn find_targets_for_current_project() -> Vec<String> {
    ...
    let runtime_dir = get_runtime_dir();
    let pattern = format!("{runtime_dir}/nvim-mcp.{escaped_project_root}.*.sock");
...
mod tests {
    ...
    #[test]
    fn test_get_runtime_dir_returns_valid_path() {
        let runtime_dir = get_runtime_dir();

        // Should return a non-empty string
        assert!(!runtime_dir.is_empty(), "Runtime directory should not be empty");

        // Should be an absolute path
        let path = std::path::Path::new(&runtime_dir);
        assert!(path.is_absolute(), "Runtime directory should be an absolute path");
    }

    #[test]
    #[cfg(unix)]
    fn test_get_runtime_dir_unix_returns_xdg_or_tmp() {
        let runtime_dir = get_runtime_dir();

        // On Unix, should be either XDG_RUNTIME_DIR or /tmp
        let is_xdg = std::env::var("XDG_RUNTIME_DIR")
            .ok()
            .map(|xdg| runtime_dir == xdg)
            .unwrap_or(false);

        let is_tmp = runtime_dir == "/tmp";

        assert!(
            is_xdg || is_tmp,
            "Runtime directory should be either XDG_RUNTIME_DIR or /tmp, got: {}",
            runtime_dir
        );
    }
    ...
    #[test]
    #[cfg(windows)]
    fn test_get_runtime_dir_windows_returns_temp() {
        let runtime_dir = get_runtime_dir();

        // On Windows, should contain TEMP or be C:\temp
        let is_temp = std::env::var("TEMP")
            .ok()
            .map(|temp| runtime_dir == temp)
            .unwrap_or(false);

        let is_c_temp = runtime_dir == "C:\\temp";

        assert!(
            is_temp || is_c_temp,
            "Runtime directory should be TEMP or C:\\temp, got: {}",
            runtime_dir
        );
    }

    #[test]
    fn test_get_runtime_dir_consistency() {
        // Multiple calls should return the same result
        let dir1 = get_runtime_dir();
        let dir2 = get_runtime_dir();

        assert_eq!(dir1, dir2, "get_runtime_dir() should be consistent across calls");
    }
}
